...
# A2UI renderers can reject a single message that
...
class _MapsKeyEventConverter(A2uiEventConverter):
    ...
    Three responsibilities:
      ...
      3. Repair `createSurface.catalogId` when the LLM substitutes the agent
         ...
         A2UI clients reject unknown catalog IDs with `Catalog not found`.
    """
...
class BridgeInventoryExecutor(A2aAgentExecutor):
    """Executor for the bridge inventory agent with A2UI GE session setup."""

    def __init__(self, base_url: str, agent: BridgeInventoryAgent):
    ...
    async def _prepare_session(
        ...
        # When the client omits the X-A2A-Extensions
        # header there is no active catalog. Gemini Enterprise sends no A2UI
        # header but renders v0.8, so default to v0.8. Custom clients can
        # still request v0.9 explicitly.
